<?php
	...
	function sp_register_sermons(){
		...
		$labels = apply_filters('sp_sermon_labels', $labels);
		$rewrite_slug = apply_filters('sp_sermons_rewrite_slug', 'sermons');
		$args = array(
			'labels'             => $labels,
			'public'             => true,
	...
	add_action('init', 'sp_register_sermons');

	//Register Meta Boxes
	function sp_register_sermons_meta_boxes($meta_boxes){
		global $wpdb;
		$prefix = 'sermon_';
		...
		$app_token = get_sp_setting('sermon_esv_token');
		if($app_token != ''){
			$sermon_info_fields[] = array(
		...
		$sermon_info_fields = apply_filters('sp_sermon_info_fields', $sermon_info_fields, $prefix);
		...
		$sermon_media_fields = array(
			...
			array(
				...
				'id' => $prefix.'audio',
				'type' => 'file_input',
				'desc' => 'You must upload the file to mediafire and then navigate to it.'
			),
			array(
				'name' => 'Video',
			...
			array(
				...
				'id' => $prefix.'video_dl',
				'type' => 'file_input',
				'desc' => 'The download link to the video file.'
			),
		);
		$sermon_media_fields = apply_filters('sp_sermon_media_fields', $sermon_media_fields, $prefix);
		...
		$sermon_additional_fields = apply_filters('sp_sermon_additional_resources_fields', $sermon_additional_fields, $prefix);
	...
	add_filter('rwmb_meta_boxes', 'sp_register_sermons_meta_boxes', 2);

	//Download and Save Passages
	function sp_download_passages($post_id){
	...
	add_action('save_post', 'sp_download_passages', 99, 1);

	//Sermon Front End Browse By Filters
	function sp_adjust_browse_by_for_sermons($query){
	...
	add_filter('pre_get_posts', 'sp_adjust_browse_by_for_sermons');
